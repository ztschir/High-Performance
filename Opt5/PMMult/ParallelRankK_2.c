		   ...
		   MPI_Comm comm_col )
{
  ...
  */
  ...
  // Compute which column of nodes owns the p-th column of A
    curcol = (pp + x) % ncols;
  int offset_x = x*sizeof(double)*local_m;
  ...
  if ( curcol == mycol ){
    local_p_A = (pp + x) / ncols;
    for ( i=0; i<local_m; i++ )
      work_A[ i + offset_x ] = LOCAL_A( i, local_p_A );
  }

  // Broadcast the copy of the local column of A within this node's row of nodes
  MPI_Bcast( &work_A[offset_x], local_m, MPI_DOUBLE, curcol, comm_row );
  }
