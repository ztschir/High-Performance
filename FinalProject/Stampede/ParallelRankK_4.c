		   ...
		   MPI_Comm comm_col )
{
  ...
  */
  ...
  work_A2 = ( double *) malloc( sizeof( double ) * local_m *bk);
  ...
  work_B2 = ( double *) malloc( sizeof( double ) * local_n *bk);

int a;
for(a=0; a<local_m*bk; a++)
{
	work_A[a] = 0;
}
for(a=0; a<local_n*bk; a++)
{
	work_B[a] = 0;
}
...
//		local_n, work_A[i+offset]);
 ...
 // MPI_Bcast( &(work_A[offset_a]), local_m, MPI_DOUBLE, curcol, comm_row );
	...
	MPI_Allreduce(work_A, work_A2, local_m*bk, MPI_DOUBLE, MPI_SUM, comm_row); 
p=x;
for(x=p; p<x+bk && p<global_k ; p++)
{
...
//  MPI_Bcast( &(work_B[offset_b]), local_n, MPI_DOUBLE, currow, comm_col );
  ...
  MPI_Allreduce(work_B, work_B2, local_n*bk, MPI_DOUBLE, MPI_SUM, comm_col); 
...
//	 local_C, &local_ldimC );
	...
	dgemm_("N", "T", &local_m, &local_n, &bk, &d_one, work_A2, &local_m, work_B2, &local_n, &d_one, local_C, &local_m); 
