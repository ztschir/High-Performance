...
// Indicate whether to check correctness against sequential run
#define CHECK 0
...
int main(int argc, char **argv )
{
  ...
  double
    ...
    *local_C_ref,    /* array in which to hold local part of matrix C */
    ...
    dtime,           /* elapsed time */
    gflops,          /* rate of computation in GFLOPS */
    gflops_per_core; /* rate of computation in GFLOPS per core */
  MPI_Comm
  ...
  /* Create an approximately square mesh */
  nprows = 1; npcols = nprocs;
  while ( nprows*2 <= npcols ){
    nprows *= 2;
    npcols /= 2;
  }
  ...
  for ( n = NFIRST; n <= NLAST; n += NINC ){ 
    ...
    random_matrix( local_m, local_n, local_A, local_m );
    random_matrix( local_m, local_n, local_B, local_m );
    random_matrix( local_m, local_n, local_C, local_m );
#endif
    ...
    /* Report various data */
    gflops = 2.0 * n * n * n / dtime * 1.0e-9;
    gflops_per_core = gflops / nprocs;

    if ( me == 0 )
      printf("%d %d %d %d %le %le %le\n", nprocs, nprows, npcols, n, dtime, gflops, gflops_per_core );
